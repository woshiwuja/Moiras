...
void Structure::snapToGround(const Model &ground) {
  ...
  for (int m = 0; m < ground.meshCount; m++) {
    RayCollision meshHitInfo =
        GetRayCollisionMesh(ray, ground.meshes[m], ground.transform);
...
void Structure::updateBounds() {
  if (modelInstance.isValid()) {
    ...
    Vector3 size = {(bounds.max.x - bounds.min.x) * scale,
                    (bounds.max.y - bounds.min.y) * scale,
                    (bounds.max.z - bounds.min.z) * scale};

    bounds.min = {position.x - size.x * 0.5f, position.y,
                  position.z - size.z * 0.5f};
    bounds.max = {position.x + size.x * 0.5f, position.y + size.y,
                  position.z + size.z * 0.5f};
  }
}
...
void Structure::setSharedShader(Shader shader) { sharedShader = shader; }
