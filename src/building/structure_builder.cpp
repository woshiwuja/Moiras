...
StructureBuilder::StructureBuilder()
    : GameObject("StructureBuilder"),
      m_buildingMode(false),
      m_selectedAsset(-1),
      m_lastSelectedAsset(-1),
      m_previewRotationY(0.0f),
      m_previewScale(1.0f),
...
void StructureBuilder::setMap(Map* map) {
    m_map = map;
}

void StructureBuilder::setCamera(Camera3D* camera) {
    m_camera = camera;
}
...
void StructureBuilder::enterBuildingMode() {
    if (m_selectedAsset >= 0 && m_selectedAsset < (int)m_assetFiles.size()) {
        ...
        TraceLog(LOG_INFO, "Entered building mode with asset: %s", m_assetFiles[m_selectedAsset].c_str());
    }
}
...
void StructureBuilder::rotatePreview(float deltaY) {
    ...
    while (m_previewRotationY > 2 * PI) m_previewRotationY -= 2 * PI;
    while (m_previewRotationY < 0) m_previewRotationY += 2 * PI;
}
...
void StructureBuilder::draw() {
    ...
    Vector3 size = {
        (bounds.max.x - bounds.min.x) * m_previewScale,
    ...
    DrawCubeWires(
        {m_previewPosition.x, m_previewPosition.y + size.y * 0.5f, m_previewPosition.z},
...
void StructureBuilder::refreshAssetList() {
    loadAssetList();
}
...
void StructureBuilder::updatePreviewPosition() {
    ...
    for (int m = 0; m < m_map->model.meshCount; m++) {
        RayCollision meshHitInfo = GetRayCollisionMesh(ray, m_map->model.meshes[m], m_map->model.transform);
...
bool StructureBuilder::checkPlacementValidity() {
    ...
    if (m_navMesh) {
        ...
        bool onNavMesh = m_navMesh->projectPointToNavMesh(m_previewPosition, projectedPoint);
...
void StructureBuilder::loadPreviewShaders() {
    ...
    m_previewShadersLoaded = (m_previewShaderValid.id != 0 && m_previewShaderInvalid.id != 0);
...
void StructureBuilder::applyPreviewShader(bool valid) {
    if (!m_previewShadersLoaded || !m_previewModelLoaded) return;
...
void StructureBuilder::loadOrGeneratePreviewTexture(const std::string& assetFilename) {
...
void StructureBuilder::generatePreviewTexture(const std::string& assetFilename, const std::string& previewPath) {
    ...
    m_previewCamera.position = {center.x + distance, center.y + distance * 0.5f, center.z + distance};
