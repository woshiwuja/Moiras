...
void Game::setup() {
  ...
  audioManager->loadMusic("meadow", "../assets/audio/meadow.mp3");
  audioManager->playMusic("meadow");
  ...
  player->name = "Player";
  player->loadModel(modelManager, "../assets/robot2.glb");
  ...
  if (mapPtr && playerPtr) {
    playerController = std::make_unique<CharacterController>(
        playerPtr, &mapPtr->navMesh, &mapPtr->model);
